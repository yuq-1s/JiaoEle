...
from cracker.without_browser.utils import asp_params
...
class Main(cmd.Cmd):
    ...
    def do_qnfull(self, _):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
            ...
            # future2name = {executor.submit(self.is_full,c):\
            #         parse_qs(urlparse(c['url']).query)['kcdm'][0]\
            #         for c in self.courses}
            # for future in concurrent.futures.as_completed(future2name):
            #     result = future.result()
            #     if result:
            #         print(future2name[future])
            # # nfull = executor.map(is_full, self.courses)
